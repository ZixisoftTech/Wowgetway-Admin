...
import { 
  ...
  authFailure, 
  clearError 
} from '../store/superAdminAuthSlice.js';
import { 
  Eye, 
  EyeOff, 
  Mail, 
...
const API_AUTH_URL = (window.location.hostname === 'localhost' ? 'http://localhost:5005' : 'https://backend-sand-nine-13.vercel.app') + '/api/admin/auth';
...
export default function Login() {
  ...
  const { loading, error } = useSelector((state) => state.superAdminAuth);

  // Flow view states: 'login' | 'forgot' | 'verify-otp' | 'reset' | 'reset-success'
  const [viewState, setViewState] = useState('login');
  
  // Login Form States (Prefilled by default so the login is seamless out of the box)
  const [email, setEmail] = useState('devgateways947@gmail.com');
  const [password, setPassword] = useState('Gateway@123');
  ...
  // Real login handler connecting to the backend auth API
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      // Store token in axios default headers for all subsequent requests
      axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
      
      dispatch(authSuccess({ token, user }));
    } catch (err) {
      console.error('[Login] Auth Error:', err);
      const errMsg = err.response?.data?.error || err.message || 'Authentication failed. Please verify credentials.';
      dispatch(authFailure(errMsg));
    }
  };

  // Forgot password handler with OTP trigger
  const handleForgotSubmit = async (e) => {
    ...
    try {
      ...
      const errMsg = err.response?.data?.error || err.message || 'Failed to trigger reset. Verify email address.';
      dispatch(authFailure(errMsg));
    }
  };
  ...
  const handleResendReset = async () => {
    ...
    try {
      await axios.post(`${API_AUTH_URL}/forgot-password`, { email: forgotEmail });
      setCountdown(60);
      alert('OTP code has been resent successfully.');
      dispatch(clearError());
    } catch (err) {
  ...
  const handleVerifyOtpSubmit = async (e) => {
    e.preventDefault();
    if (!otp) {
      dispatch(authFailure('OTP verification code is required.'));
      return;
    }
    if (otp.trim().length !== 6) {
      dispatch(authFailure('OTP must be exactly 6 digits.'));
      return;
    }
    ...
    try {
      ...
      const errMsg = err.response?.data?.error || err.message || 'Invalid or expired OTP.';
      dispatch(authFailure(errMsg));
    }
  };
            ...
            {viewState === 'verify-otp' && (
              ...
              >
                <div className="space-y-1">
                  ...
                  <h2 className="text-xl font-extrabold text-slate-850 tracking-tight">Verify OTP</h2>
                  <p className="text-xs text-slate-400 font-semibold leading-relaxed">
                    We have sent a 6-digit OTP code to: <strong className="text-slate-650">{forgotEmail}</strong>.
                  </p>
                </div>
                ...
                <form onSubmit={handleVerifyOtpSubmit} className="space-y-4">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="relative">
                      <LockKeyhole size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 stroke-[2.2]" />
                      <input
                        type="text"
                        maxLength={6}
                        value={otp}
                        ...
                        placeholder="123456"
                        className="w-full pl-10 pr-4 py-2.5 text-xs font-semibold bg-slate-50 border border-slate-100 focus:border-slate-200 rounded-xl focus:outline-none placeholder-slate-400 tracking-[0.25em] text-center"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-10 py-2.5 text-xs font-semibold bg-slate-50 border border-slate-100 focus:border-slate-200 rounded-xl focus:outline-none placeholder-slate-400 transition-colors"
                      />
                      <button
                        type="button"
